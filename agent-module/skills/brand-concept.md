...
요청이 촬영 컨셉/무드/룩이고 `.ff`·`.art`가 없으면 **12섹션**만 (Gap-first):
...
**내부 도출 순서:** 열망 갭 → 기능 갭 → 캐릭터 → Loadout → Scene. 캐릭터는 갭 다음 필수 섹션.
...
- 「`.art` 드릴까요?」 같은 CTA, 다음 단계 권유, A/B 선택지 업셀, 확장 안내
- 영문 약어 `A-Gap`/`F-Gap`를 사용자 화면에 쓰기 (한글 **열망 갭** / **기능 갭**만)
