...
**전략 정본:** live brand manual (`ORGANIZATION BRAND MANUAL` injected this turn) — Gap-first (열망 갭·기능 갭) · PAA · 라인 언어  
...
| 트리거 | 출력 |
|--------|------|
| `TLP125 촬영 컨셉` / 무드 / look (`.ff`·`.art` 없음) | **CONCEPT_CORE** 12섹션 (~600–1600자) |
...
- **내부 목차 누출 금지:** 임의 `§5-1`·`§5-2`·자작 절번호·영문 약어(`A-Gap`/`F-Gap`)를 사용자 화면에 쓰지 말 것 — 한글 **열망 갭** / **기능 갭**은 CONCEPT_CORE 섹션으로 허용
- 사용자가 `.ff` / `.art` / `.dev` / `.ops` / 3-Layer 정리를 **명시하면** 그 형식은 **완전하게** 제공
- 기본 `.art` = 메인 1장만

---

## CONCEPT_CORE (기본 — 12섹션)

🏷️ 컨셉명 → 💬 `영문 슬로건:` (12–18자) → ✨ 한 줄 정의 → 🎯 열망 갭 → ⚙️ 기능 갭 → 👤 캐릭터 → 🌅 핵심 무드 → 🎨 비주얼 키워드 → 🖼️ 컨셉 이미지 방향 → 🌍 라인·세계관 → 📽️ 무드 참고 → 🧵 CQR 연결 (**여기서 끝**)

**내부 도출 순서 (Gap-first):** 열망 갭 → 기능 갭 → **캐릭터** → Loadout → Scene. 캐릭터 섹션은 갭 다음 **필수 출력** (이미지 방향에만 묻히지 말 것).
...
- **Gap-first** — 열망 갭·기능 갭을 Character보다 먼저 잠금 (전략서 v3.2.5 라인 표)
...
**정본 위치:** 상세 규칙·번들 원본은 `cqr_brand_manager/prompt_concept/`. 이 모듈에는 MY Agent 주입용 `AGENTS.md` + `MY_prompt.md`만 둔다.
