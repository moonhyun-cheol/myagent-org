...
0. **Live brand manual** injected this turn (`ORGANIZATION BRAND MANUAL` from `http://hub.example.internal:8080/api/brand-manual/current.md`). Brand philosophy, PAA, line language: this wins over static snapshots.
1. CQR_INTERNAL_STRATEGY_v3.1.md — philosophy hierarchy, PAA dual gap, 8 value axes, LEGACY two-layer, 3-layer, copy/visual guardrails. Prefer v3.1 over any v2.x file.
2. model_row_index.txt — primary model match table
3. PRODUCT_DEV_SPEC_ENGINE.md — new product spec matrices and ISSUE constraints
4. COLOR_CODE.md — color code to color name
5. cqr_development_direction.txt or latest development-direction extract
6. CQR_BRAND_CONCEPT.md
7. CQR_LOADOUT_SYSTEM.md — line-neutral loadout registry (LO-MOV/OBS/SIG/CMD/CARE/INS/TRN; G1/G2/G3 alias)
8. SCENE_BRIEF_ENGINE.md
9. CQR_VISUAL_DNA.md — listing image analysis and AI prompt slots
10. CQR_PROMPT_GENERATION_PROTOCOL.md — **`.art` assembly method**, whitelist locations, constraint lock card
11. CQR_BRAND_IMAGE_PLAYBOOK.md — brand image concierge role, intent router, QC, compass
12. CQR_FILM_MOOD_REF.md — **mandatory block** for FULL briefs; never copy to `.art`
13. GOLDEN_EXAMPLE.md — format reference only
14. CQR_LINEUP_V31_OVERLAY.md — 신제품·애매 SKU overlay
15. BRAND_INDEX.md for lane overview

If knowledge and user input conflict, prefer the live brand manual, then the newest user-supplied material, then development-direction extract, then catalog snapshot.
